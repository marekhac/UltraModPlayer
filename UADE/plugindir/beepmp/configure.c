 ...
 *
 * This plugin is based on bmp 0.9.6 wavplayer input plugin code. Since
 ...
 *
 * use of the standard bmp configuration file to load/store state of the 
 * options
 */
...
void uade_configread(void)
{
  ...
  xmms_cfg_read_boolean(cfg, "uade", "use_slider", &use_slider);
...
void uade_configwrite(void)
{
  ...
  xmms_cfg_write_boolean(cfg, "uade", "use_slider", use_slider);
...
/* The GTK Dialog for the uade bmp configure plugin */
...
//Start of page 2
    ...
    gtk_tooltips_set_tip(tooltips, use_songend_button,
			 "some replayers message when a song is over instead of playing it infinetely\nif enabled uade will skip to the next subsong and/or song in the bmp playlist.\n(default: enabled)",
			 NULL);
...
//Start of Compatibility frame, text and option widgets
    ...
    gtk_tooltips_set_tip(tooltips, filemagic_decr_button,
			 "useful if you also want to use the detection by contents with packed files like *.gz, *.zip, etc.\nWarning: it slows down adding files to the bmp playlist, which can be very annyoing if you try to load files over a network\n(default: disabled)",
			 NULL);
...
/* Subsongseeker frame */
    ...
    use_slider_button =
	gtk_check_button_new_with_label
	("\ndisable uade's subsong changer and use \nbmp`s slider bar to change subsongs.");
    gtk_signal_connect(GTK_OBJECT(use_slider_button), "clicked",
		       GTK_SIGNAL_FUNC(arrows_on_off_toggle), NULL);
    gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(use_slider_button),
				 use_slider);
    ...
    gtk_box_pack_start(GTK_BOX(conf_ui_vbox), use_slider_button,
		       FALSE, FALSE, 0);
...
static void arrows_on_off_toggle()
{
  if (gtk_toggle_button_get_active
      (GTK_TOGGLE_BUTTON(use_slider_button))) {
...
void formatsfile_edit(void)
{
  ...
  if (!formatswin) {
    ...
    formatswin = gtk_window_new(GTK_WINDOW_TOPLEVEL);
...
{
  char *p[3];
  p[0] = (char *) gtk_entry_get_text(GTK_ENTRY(formats_ext_entry));
  p[1] = (char *) gtk_entry_get_text(GTK_ENTRY(formats_player_entry));
...
//    fprintf (stderr, "saving pref/ext and players to: %s\n",curr_formatsfilename);
  ...
  /* write header */
  fprintf(formatsfile, "# File generated by the UADE bmp plugin.\n"
...
static void formatsfile_to_roxmime()
{
  ...
  if (formatsfile == 0) {
    ...
    /* write Entries */
    fprintf(roxfile,
	    "# ROX 1.2x MIME-info\n# generated by the uade bmp plugin\n# UADE: http://uade.ton.tut.fi\n\n");
    fprintf(roxfile, "%s\n", ROX_MIMETYPE);
...
static void formatsfile_to_smimime()
{
  ...
  if (formatsfile == 0) {
    ...
    fprintf(smifile,
	    "<!-- generated by the UADE bmp input plugin     -->\n");
    fprintf(smifile,
	    "<!-- UADE: http://uade.ton.tut.fi    -->\n\n");
