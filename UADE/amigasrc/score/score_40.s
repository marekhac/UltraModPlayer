...
epcheck3warn	dc.b	'warning: using DTP_Check3',0
	even

* check module: Priority for checks (first is highest):
...
initsound	lea	eaglebase(pc),a5
	...
	bne.b	hasinitsound
	lea	initsoundmis(pc),a0
	bsr	put_string
	moveq	#0,d0
	bra.b	initsoundintena
hasinitsound	move.l	d0,a0
...
initsoundmis	dc.b	'Stupid deliplayer: does not have init sound',0
	even
...
exec_findtask	pushr	a0
	lea	findtaskmsg(pc),a0
	bsr	put_string
	pullr	a0
	move.l	#$deadface,d0
	rts

findtaskmsg	dc.b	'exec.library/findtask() not good',0
allocsigmsg	dc.b	'exec.library/allocsignal() not good',0
	even
exec_sig_mask	dc.l	$0000ffff
exec_allocsignal	pushr	a0
	lea	allocsigmsg(pc),a0
	bsr	put_string
	pullr	a0
	push	d1/a0
