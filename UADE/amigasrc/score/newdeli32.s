...
killuserinterrupts	push	all
	* should we kill ciab interrupt?
	* kill timer device *
	lea	vblanktimerstatusbit(pc),a0
	clr.l	(a0)
	* kill vbi list *
...
inputmsg	dc.l	SHD_GENERALMSG
	dc.b	'sound core got unknown input message 0x'
...
mylevel4_2	push	all
	...
	move	intenar(a2),d2
	btst	#14,d2
	bne.b	mylevel4_2_ints_enabled
	lea	mylevel4_2_dismsg(pc),a0
	moveq	#mylevel4_2_dismsge-mylevel4_2_dismsg,d0
	bsr	putmessage
	pull	all
	rte
mylevel4_2_dismsg
	dc.l	SHD_GENERALMSG
	dc.b	'audio interrupt taken but interrupts enabled.. hmm.. '
	dc.b	'please report this!',0
mylevel4_2_dismsge
	even
mylevel4_2_ints_enabled
	lea	irqlines(pc),a3
	lea	isdatapointers(pc),a4
mylevel4_2_beg	move.l	#$0780,d2
	and	intenar(a2),d2
	and	intreqr(a2),d2
	moveq	#0,d7
mylevel4_2_loop	move	mylevel4_2_int_seq(pc,d7),d6	* aud0int bit
	btst	d6,d2
	beq.b	mylevel4_2_no_int
	move.l	d6,d1
	lsl	#2,d1
	move.l	(a3,d1),d0
	beq.b	mylevel4_2_no_int
	...
	bsr	mylevel4regc
	cmp.l	d0,d1
	beq.b	mylevel4_2_not_fail
	push	all
	lea	mylevel4_2_msg(pc),a0
	moveq	#mylevel4_2_msge-mylevel4_2_msg,d0
	bsr	putmessage
	pull	all
mylevel4_2_not_fail
	bra.b	mylevel4_2_beg
mylevel4_2_no_int
	addq	#2,d7
	cmp	#4*2,d7
	bne.b	mylevel4_2_loop
	pull	all
	rte

mylevel4_2_int_seq
	dc	8, 10, 7, 9	* order of audio interrupt execution

mylevel4_2_msg	dc.l	SHD_GENERALMSG
	dc.b	'audio interrupt checksum failure: '
	dc.b	'please report this!',0
mylevel4_2_msge	even
