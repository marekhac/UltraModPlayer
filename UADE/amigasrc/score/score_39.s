...
initvolume	move.l	volumefunc(pc),d0
	beq.b	novolfunc
	lea	eaglebase(pc),a5
	move.l	d0,a0
	jsr	(a0)
...
tagloop	move.l	(a0),d0
	...
	lea	dtp_check(pc),a1
	move.l	4(a0),(a1)
nono1	cmp.l	#EP_Check5,d0
	bne.b	nono1_2
	lea	ep_check(pc),a1
	move.l	4(a0),(a1)
nono1_2	cmp.l	#DTP_Interrupt,d0
	bne.b	nono2
	lea	intfunc(pc),a1
	move.l	4(a0),(a1)
...
initplayer	lea	eaglebase(pc),a5
...
* check module:	if dp_check is available use it, otherwise if ep_check is
*	available use that
check_module	lea	eaglebase(pc),a5
	move.l	dtp_check(pc),d0
	sne	d2
	move.l	ep_check(pc),d1
	sne	d3
	and.b	d2,d3
	beq.b	only_one_check
	pushr	d0
	lea	checkwarn(pc),a0
	bsr	put_string
	pullr	d0
...
hunksizewarnmsg	dc.b	'hunk size warning',0
hunk3f7warnmsg	dc.b	'hunk 3f7 warning',0
symbolhunkwarnmsg	dc.b	'hunk relocator: symbol hunk warning!',0
illegalhunkmsg	dc.b	'illegal hunk',0
	even
...
amplifier_init	move.l	amplifier_init_func(pc),d0
	bne.b	is_an_amp
	rts
is_an_amp	push	all
...
amplifier_adr	push	all
	...
	mulu	#amp_entry_size,d1
	lea	amp_struct(pc),a0
	moveq	#AMP_ADR_CHANGED,d2
	move.l	d2,d3
	and.l	amp_changes(a0,d1),d3
	beq.b	amp_adr_no_prob
	nop
amp_adr_no_prob	move.l	d0,amp_adr(a0,d1)
	or.l	d2,amp_changes(a0,d1)
	pull	all
	rts
...
amplifier_len	push	all
	...
	mulu	#amp_entry_size,d1
	lea	amp_struct(pc),a0
	move.l	d0,amp_len(a0,d1)
	or.l	#AMP_LEN_CHANGED,amp_changes(a0,d1)
	pull	all
	rts
...
amplifier_per	push	all
	...
	mulu	#amp_entry_size,d1
	lea	amp_struct(pc),a0
	move	d0,amp_per(a0,d1)
	or.l	#AMP_PER_CHANGED,amp_changes(a0,d1)
	pull	all
	rts
...
amplifier_vol	push	all
	...
	mulu	#amp_entry_size,d1
	lea	amp_struct(pc),a0
	moveq	#AMP_VOL_CHANGED,d2
	move.l	d2,d3
	and.l	amp_changes(a0,d1),d3
	beq.b	amp_no_vol_prob
	nop
amp_no_vol_prob	move	d0,amp_vol(a0,d1)
	or.l	#AMP_VOL_CHANGED,amp_changes(a0,d1)
	pull	all
	rts
...
amplifier_int	push	all
	move	amp_dma(pc),d0
	beq.b	amp_no_dma_disabling
	move	d0,dmacon+custom
amp_no_dma_disabling

	lea	amp_struct(pc),a0
	lea	$dff000,a2
	moveq	#4-1,d7
amp_loop	move.l	amp_changes(a0),d0
	beq.b	amp_nothing
	btst	#0,d0
	beq.b	amp_no_adr
	move.l	amp_adr(a0),aud0lch(a2)
amp_no_adr	btst	#1,d0
	beq.b	amp_no_len
	move.l	amp_len(a0),d1
	move	d1,aud0len(a2)
amp_no_len	btst	#2,d0
	beq.b	amp_no_per
	move	amp_per(a0),aud0per(a2)
amp_no_per	btst	#3,d0
	beq.b	amp_no_vol
	move	amp_vol(a0),aud0vol(a2)
amp_no_vol
amp_nothing	add	#amp_entry_size,a0
	add	#$10,a2
	dbf	d7,amp_loop

	move	amp_dma+2(pc),d0
	beq.b	amp_no_dma_enabling
	and	#$000f,d0
	move	d0,dmacon+custom
	bsr	wait_audio_dma
	or	#$8000,d0
...
debug_info	debuginfo	'uade debug info', debug_info
	debuginfo	'check module', check_module
	debuginfo	'init player', initplayer
	debuginfo	'check sub songs', CheckSubSongs
	debuginfo	'init volume', initvolume
	debuginfo	'init sound', initsound
	debuginfo	'start int', startint
	debuginfo	'interrupt', trapcall
	debuginfo	'amp init', amplifier_init
